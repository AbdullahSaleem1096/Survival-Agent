...
void ServiceMain(int argc, char** argv) {
  ...
  // 1. Tell Windows we are officially RUNNING first.
  // This stops the "START_PENDING" hang.
  serviceStatus.dwCurrentState = SERVICE_RUNNING;
  SetServiceStatus(hStatus, &serviceStatus);

  // 2. Now launch the agent. 
  // Note: If agent.exe is a GUI app (like a MessageBox), it might run 
  // in Session 0 (background). That is normal for Services.
  RunMeow(); 
